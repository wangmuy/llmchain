...
import com.wangmuy.llmchain.utils.queue.PriorityQueue
import com.wangmuy.llmchain.utils.uuid.SimpleUUID
...
class SimpleVectorStore(
    ...
    interface DistanceFunc {
        fun invoke(emb1: Array<Float>, emb2: Array<Float>): Float
    }
    ...
    class SimpleIndex(private val collectionName: String): Index {
            ...
            fun findNearestNeighbors(
                ...
                val queue = PriorityQueue<QueueNode> { node1, node2 ->
                    val d1 = distanceMap.getOrPut(node1) {
                        distanceFunc.invoke(embedding, node1.embedding)
                    }
                    val d2 = distanceMap.getOrPut(node2) {
                        distanceFunc.invoke(embedding, node2.embedding)
                    }
                ...
                while(queue.size > 0) {
                    ...
                    distList.add(distanceMap.getOrPut(emb) {
                        distanceFunc.invoke(embedding, emb.embedding)
                    })
                }
        ...
        override fun getNearestNeighbors(
            ...
            val collection = collection.filterKeys { it in ids }
    ...
    companion object {
        ...
        private fun getOrCreateCollection(
            ...
            return COLLECTIONS.getOrPut(collectionName) {
                val uuid = SimpleUUID.randomUUID().toString() // uuid4
    ...
    override fun addTexts(
        ...
        // uuid1
        val ids = (args?.get(KEY_IDS) ?: texts.map { SimpleUUID.randomUUID().toString() }.toList()) as List<String>
    ...
    private fun getNearestNeighbors(
        ...
        val embList = EMBEDDINGS.getOrPut(collection.name) { mutableListOf() }.filter {emb->
